    ...
    public void run(int nRunsPerDataset, boolean runPython) {
        ...
        for (int i = 0; i < observedPaths.length; i++) {
            ...
            for (int j = 0; j < nRunsPerDataset; j++) {
                ...
                Config config = new Config(false, false, "Default", "Default", a, 0.0, 0, false, false);
    ...
    public void runWIthUI(Config config,
        ...
        for (int i = 0; i < observedPaths.length; i++) {
            ...
            for (int j = 0; j < nRunsPerDataset; j++) {
                ...
                Config newConfig = new Config(config.isUseAlternateConsistencyCheck(), config.isUseWernerModification(), config.getCutGeneratorName(), config.getCostFunctionName(), a, config.getaFactor(), config.getPsi(), false, false);
